...
# ── REMOVED: smtplib, ssl, email.mime  (Render blocks outbound SMTP)
# ── ADDED:   Resend HTTP API via httpx (already a dependency)
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://agile-voyager.vercel.app"
    ],
...
# From address must be a verified domain/sender in your Resend account.
# If you're using the free sandbox you can use "onboarding@resend.dev" as FROM.
RESEND_API_KEY  = os.getenv("RESEND_API_KEY")
EMAIL_FROM      = os.getenv("EMAIL_FROM", "Agile Voyager <onboarding@resend.dev>")
# EMAIL_USER kept for backward-compat checks elsewhere in the file
...
# ─────────────────────────────────────────────
#  RESEND HELPER  (replaces every smtplib block)
# ─────────────────────────────────────────────
async def send_email_via_resend(
    ...
    try:
        ...
        if response.status_code in (200, 201):
            data = response.json()
            print(f"✅ Resend: email sent to {to}, id={data.get('id')}")
            return {"success": True, "id": data.get("id")}
        else:
            error_msg = response.text
            print(f"❌ Resend error {response.status_code}: {error_msg}")
            return {"success": False, "message": f"Resend API error {response.status_code}: {error_msg}"}
...
class TravelFormData(BaseModel):
    ...
    numberOfPeople: Literal["1", "2", "3 - 6", "7 - 10", "10+"]
    rentCar: bool
    needsFlight: bool
    dob: str
    userId: str
...
def get_country_centroid(name: str):
    ...
    try:
        ...
        if latlng and len(latlng) >= 2:
            return {"lat": float(latlng[0]), "lon": float(latlng[1]),
                    "display_name": entry.get("name", {}).get("common", name)}
    except Exception as e:
    ...
    try:
        ...
        for el in elements:
            ...
            try:
                ...
                continue
            seen.add(name)
            desc_parts = [tags.get(k, "").replace("_", " ").title()
                          for k in ["tourism", "historic", "natural"] if k in tags]
            attractions.append({
                "name": name, "lat": lat_f, "lon": lon_f,
...
async def get_popular_places(request: dict):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            geocode_response = await client.get(geocode_url, headers={"User-Agent": "TravelPlanner/1.0"})
            ...
            return {"places": places_data, "activities": activities_data}
...
async def fetch_foursquare_places(lat: float, lon: float, destination: str):
    ...
    try:
        ...
        params = {"ll": f"{lat},{lon}", "radius": 10000, "categories": "16000,10000,12000", "limit": 10}
...
async def fetch_activities_by_type(lat: float, lon: float, trip_type: str, destination: str):
    category_map = {
        "adventure":  "18000,19000",
        "relaxation": "10027,10032",
        "business":   "13065,13032",
        "romantic":   "13065,10040",
        "family":     "10019,10047",
        "solo":       "10023,10018",
    }
    ...
    try:
        url    = "https://api.foursquare.com/v3/places/search"
        params = {"ll": f"{lat},{lon}", "radius": 15000, "categories": categories, "limit": 8}
        headers = {"Accept": "application/json", "Authorization": FOURSQUARE_API_KEY}
        async with httpx.AsyncClient() as client:
            response = await client.get(url, params=params, headers=headers)
            data = response.json()
...
async def get_fallback_activities(trip_type: str, destination: str):
    ...
    return activities_map.get(trip_type.lower(),
        f"Things to do in {destination}:\n• City exploration\n• Local cuisine\n• Cultural sites\n• Shopping\n• Photography")


# ─────────────────────────────────────────────
#  SEND ITINERARY  (was SMTP, now Resend)
# ─────────────────────────────────────────────
...
async def send_itinerary(data: EmailItineraryWithUserId):
    ...
    try:
        ...
        # Resolve email / user_id from DB when needed
        if not recipient_email or recipient_email == "user@example.com":
            if not data.userId:
                return {"success": False, "message": "No valid email address or user ID provided"}
            try:
                if conn and cursor:
                ...
                return {"success": False, "message": f"Failed to fetch user email: {str(db_error)}"}
        else:
            try:
                if conn and cursor:
                    ...
                    if user:
                        ...
                        return {"success": False, "message": f"No user found with email: {recipient_email}"}
            except Exception as e:
                return {"success": False, "message": "Failed to fetch user from database"}

        if not recipient_email or not user_id:
            return {"success": False, "message": "Could not determine user email or ID"}
        ...
        text_body = f"Your {data.destination} Travel Itinerary\n\n{data.itinerary}\n\nPacking List:\n{data.packingList}"

        # ── Send via Resend ──
        result = await send_email_via_resend(
            to       = recipient_email,
            subject  = f"Your {data.destination} Travel Itinerary",
            html_body= html_body,
            text_body= text_body,
        )
        ...
        if conn and cursor and user_id:
            try:
                ...
                try:
                    year = datetime.strptime(data.departureDate, "%Y-%m-%d").year
                except Exception:
                    pass

                members = 1
                try:
                    members = int(data.travelers.split()[0]) if data.travelers else 1
                except Exception:
                    pass
                ...
                itinerary_json = json.dumps({"itinerary": data.itinerary, "packing_list": data.packingList})
...
# ─────────────────────────────────────────────
#  SEND EMAIL  (was SMTP, now Resend)
# ─────────────────────────────────────────────
@app.post("/send-email")
...
"""
        for day_item in itinerary:
            ...
            for activity in day_item.get('activities', []):
                ...
                if activity.get('description'):
                    text_content += f"    {activity['description']}\n"
        ...
        """
        for day_item in itinerary:
            html_content += f'<div style="margin-bottom: 20px;"><h4 style="color: #6366f1; margin-bottom: 10px;">Day {day_item.get("day","N/A")}: {day_item.get("title","")}</h4>'
            for activity in day_item.get('activities', []):
                html_content += f"""
        ...
        if result["success"]:
            result["message"] = f"Email sent successfully to {user_email}"
        return result
...
# ─────────────────────────────────────────────
#  ALL REMAINING ENDPOINTS (unchanged logic)
# ─────────────────────────────────────────────
...
async def toggle_trip_completion(trip_id: int, completion: TripComplete):
    ...
    try:
        ...
        cursor.execute("UPDATE confirmed_trips SET completed = ? WHERE id = ?",
                       (1 if completion.completed else 0, trip_id))
        conn.commit()
        cursor.execute(
            "SELECT id, destination, total_budget, days, trip_type, members, completed, year, created_at FROM confirmed_trips WHERE id = ?",
            (trip_id,))
        updated_trip = cursor.fetchone()
        trip_dict = dict(updated_trip)
        trip_dict['completed'] = bool(trip_dict.get('completed', 0))
        return {"success": True, "message": f"Trip marked as {'completed' if completion.completed else 'pending'}", "trip": trip_dict}
...
async def get_trip_expenses(trip_id: int):
    check_db_connection()
    try:
        cursor.execute("SELECT id, destination, total_budget FROM confirmed_trips WHERE id = ?", (trip_id,))
        trip = cursor.fetchone()
        if not trip:
            raise HTTPException(status_code=404, detail="Trip not found")
        cursor.execute(
...
async def get_user_expenses(user_id: str):
    check_db_connection()
    try:
        cursor.execute("SELECT id, destination, total_budget FROM confirmed_trips WHERE user_id = ?", (user_id,))
        trips = [dict(t) for t in cursor.fetchall()]
        if not trips:
            return {"user_id": user_id, "trips": [], "total_budget": 0, "total_spent": 0}
        ...
        return {"user_id": user_id, "trips": trips, "total_budget": total_budget,
                "total_spent": total_spent, "remaining_budget": total_budget - total_spent}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def add_expense(expense: Expense):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, trip_id, category, amount, description, date, created_at FROM expenses WHERE id = ?", (new_id,))
        return {"success": True, "message": "Expense added successfully", "expense": dict(cursor.fetchone())}
    except HTTPException:
        raise
    except Exception as e:
        if conn: conn.rollback()
...
async def update_expense(expense_id: int, expense_update: ExpenseUpdate):
    ...
    try:
        ...
        updates, params = [], []
        if expense_update.category    is not None: updates.append("category = ?");    params.append(expense_update.category)
        if expense_update.amount      is not None: updates.append("amount = ?");      params.append(expense_update.amount)
        if expense_update.description is not None: updates.append("description = ?"); params.append(expense_update.description)
        if expense_update.date        is not None: updates.append("date = ?");        params.append(expense_update.date)
        if not updates:
        ...
        conn.commit()
        cursor.execute(
            "SELECT id, trip_id, category, amount, description, date, created_at FROM expenses WHERE id = ?", (expense_id,))
        return {"success": True, "message": "Expense updated successfully", "expense": dict(cursor.fetchone())}
    except HTTPException:
        raise
    except Exception as e:
        if conn: conn.rollback()
...
async def confirm_trip(data: dict):
    try:
        ...
        if departure_date:
            try:
                year = datetime.strptime(departure_date, "%Y-%m-%d").year
            except Exception:
                pass

        if not user_id:     return {"success": False, "message": "Missing userId"}
        if not destination: return {"success": False, "message": "Missing destination"}
        if not plan_text:   return {"success": False, "message": "Missing planText"}
        ...
        cursor.execute(
            "SELECT id, user_id, destination, plan_text, total_budget, days, trip_type, members, itinerary_json, year, completed, created_at FROM confirmed_trips WHERE id = ?",
            (new_id,))
        row = cursor.fetchone()
        if row:
            result = dict(row)
...
async def debug_all_data():
    ...
    try:
        ...
        expenses = [dict(e) for e in cursor.fetchall()]
        return {"users": users, "trips": trips, "expenses": expenses,
                "trip_count": len(trips), "expense_count": len(expenses)}
    except Exception as e:
        return {"error": str(e)}
...
async def get_latest_confirmed_trip_by_email(email: str):
    ...
    try:
        ...
        count = (cursor.fetchone() or {}).get("count", 0)
        if count == 0:
        ...
        cursor.execute(
            """SELECT id, user_id, destination, plan_text, total_budget, days,
               ...
               FROM confirmed_trips WHERE user_id = ? ORDER BY created_at DESC LIMIT 1""",
            (user_id,))
        trip = cursor.fetchone()
        if not trip:
            raise HTTPException(status_code=404, detail="No confirmed trip found")
...
async def get_weather_forecast(weather_request: WeatherRequest, days: int = 7):
    ...
    url      = f"https://api.openweathermap.org/data/2.5/forecast?q={weather_request.city}&appid={OPENWEATHER_API_KEY}&units=metric&cnt={days * 8}"
    ...
    for item in data['list']:
        ...
        if date not in daily_forecasts:
            daily_forecasts[date] = {'temp': [], 'description': item['weather'][0]['description'],
                                     'humidity': item['main']['humidity'], 'wind_speed': item['wind']['speed']}
        daily_forecasts[date]['temp'].append(item['main']['temp'])
    forecast_list = [
        {'date': date, 'temperature': sum(info['temp']) / len(info['temp']),
         'description': info['description'], 'humidity': info['humidity'], 'wind_speed': info['wind_speed']}
        for date, info in daily_forecasts.items()
    ]
    return {'city': weather_request.city, 'forecasts': forecast_list[:days]}
...
async def get_weather(weather_request: WeatherRequest):
    ...
    d = r.json()
    return WeatherResponse(city=d["name"], temperature=d["main"]["temp"],
...
async def create_travel_plan(form_data: TravelFormData):
    try:
        ...
        for i in range(max_places):
            p = attractions[i]
            itinerary.append(Attraction(name=p["name"], lat=p["lat"], lng=p["lon"],
...
async def get_feedbacks():
    ...
    try:
        ...
        return [{"message": r["message"], "user_name": r["user_name"]} for r in cursor.fetchall()]
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        cursor.execute(
            """SELECT id, destination, total_budget, days, trip_type, members,
               ...
               FROM confirmed_trips WHERE user_id = ?
        ...
        for trip in trips:
            ...
            if not trip_dict.get('year'):
                try:
                    trip_dict['year'] = datetime.strptime(trip_dict['created_at'], '%Y-%m-%d %H:%M:%S').year
                except Exception:
                    trip_dict['year'] = datetime.now().year
            trip_dict['completed'] = bool(trip_dict.get('completed', 0))
            trips_list.append(trip_dict)
        return {"user_id": user_id, "trips": trips_list, "count": len(trips_list),
...
async def update_trip(trip_id: int, trip_update: TripUpdate):
    ...
    try:
        ...
        if not cursor.fetchone():
            raise HTTPException(status_code=404, detail="Trip not found")
        updates, params = [], []
        if trip_update.destination  is not None: updates.append("destination = ?");  params.append(trip_update.destination)
        if trip_update.total_budget is not None: updates.append("total_budget = ?"); params.append(trip_update.total_budget)
        if trip_update.days         is not None: updates.append("days = ?");         params.append(trip_update.days)
        if trip_update.trip_type    is not None: updates.append("trip_type = ?");    params.append(trip_update.trip_type)
        if trip_update.members      is not None: updates.append("members = ?");      params.append(trip_update.members)
        if trip_update.completed    is not None: updates.append("completed = ?");    params.append(1 if trip_update.completed else 0)
        if trip_update.year         is not None: updates.append("year = ?");         params.append(trip_update.year)
        if not updates:
            raise HTTPException(status_code=400, detail="No fields to update")
        params.append(trip_id)
        cursor.execute(f"UPDATE confirmed_trips SET {', '.join(updates)} WHERE id = ?", params)
        conn.commit()
        cursor.execute(
            "SELECT id, destination, total_budget, days, trip_type, members, completed, year, created_at FROM confirmed_trips WHERE id = ?",
            (trip_id,))
        trip_dict = dict(cursor.fetchone())
        trip_dict['completed'] = bool(trip_dict.get('completed', 0))
        return {"success": True, "message": "Trip updated successfully", "trip": trip_dict}
    except HTTPException:
        raise
    except Exception as e:
        if conn: conn.rollback()
...
async def get_user_achievements(user_id: str):
    ...
    try:
        cursor.execute(
            "SELECT id, destination, days, trip_type, year FROM confirmed_trips WHERE user_id = ? AND completed = 1 ORDER BY year ASC",
            (user_id,))
        ...
        achievements = []
        if total_trips >= 10:
            achievements.append({"icon": "🌟", "title": "Legendary Explorer", "subtitle": f"{total_trips} trips completed", "color": "from-purple-400 to-pink-500", "unlocked": True})
        elif total_trips >= 5:
            achievements.append({"icon": "🌍", "title": "World Traveler",     "subtitle": f"{total_trips} trips completed", "color": "from-blue-400 to-indigo-500",  "unlocked": True})
        elif total_trips >= 3:
            achievements.append({"icon": "🗺️", "title": "Cultural Explorer",  "subtitle": f"{total_trips} trips completed", "color": "from-green-400 to-emerald-500","unlocked": True})
        elif total_trips >= 1:
            achievements.append({"icon": "✈️", "title": "First Journey",      "subtitle": "1 trip completed",               "color": "from-teal-400 to-cyan-500",    "unlocked": True})
        if solo_trips >= 3:
            achievements.append({"icon": "🎒", "title": "Solo Adventurer",    "subtitle": f"{solo_trips} solo trips",       "color": "from-orange-400 to-red-500",   "unlocked": True})
        elif solo_trips >= 1:
            achievements.append({"icon": "💼", "title": "Independent Traveler","subtitle": "First solo trip",               "color": "from-yellow-400 to-orange-400","unlocked": True})
        if group_trips >= 3:
            achievements.append({"icon": "👥", "title": "Group Leader",       "subtitle": f"{group_trips} group trips",     "color": "from-teal-400 to-cyan-500",    "unlocked": True})
        elif group_trips >= 1:
            achievements.append({"icon": "🧭", "title": "Team Explorer",      "subtitle": "First group trip",              "color": "from-emerald-400 to-teal-400", "unlocked": True})
        if total_days   >= 30:
            achievements.append({"icon": "⏰", "title": "Time Traveler",      "subtitle": f"{total_days} days traveled",   "color": "from-indigo-400 to-purple-500","unlocked": True})
        if unique_years >= 3:
            achievements.append({"icon": "📅", "title": "Annual Explorer",    "subtitle": f"{unique_years} years active",  "color": "from-pink-400 to-rose-500",    "unlocked": True})
        return {"user_id": user_id, "achievements": achievements,
                "statistics": {"total_trips": total_trips, "solo_trips": solo_trips,
                               "group_trips": group_trips, "total_days": total_days, "unique_years": unique_years}}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def register_user(user: UserSignUp):
    ...
    try:
        ...
        cursor.execute("INSERT INTO users (name, email, dob, password_hash) VALUES (?, ?, ?, ?)",
...
async def signin(user: UserSignIn):
    ...
    try:
        ...
        dob_val = db_user["dob"] if isinstance(db_user, dict) else (db_user["dob"] if "dob" in db_user.keys() else db_user[3])
        return {"status": "success", "message": "Sign in successful",
                "user": {"id": db_user["id"], "name": db_user["name"], "email": db_user["email"], "dob": dob_val},
...
async def get_users():
    ...
    try:
        ...
        return {"users": [dict(u) for u in cursor.fetchall()]}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_user_profile(user_id: int):
    ...
    try:
        ...
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        return {"id": user["id"], "email": user["email"], "name": user["name"], "username": user["name"]}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
